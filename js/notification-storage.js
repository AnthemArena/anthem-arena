...
// ========================================
// GET AVAILABLE ACTIONS (for privacy checks)
// ========================================

export async function getAvailableActions(targetUserId) {
    try {
        const currentUserId = localStorage.getItem('tournamentUserId');
        ...
        if (!currentUserId || currentUserId === 'anonymous') {
            return {
                canMessage: false,
                messageReason: 'Login required'
            };
        }
        
        // Get target user's privacy settings
        const { getUserPrivacySettings } = await import('./privacy-helper.js');
        const privacySettings = await getUserPrivacySettings(targetUserId);
        
        // Check message permission
        let canMessage = false;
        let messageReason = '';
        
        const messageSetting = privacySettings?.messagePrivacy || 'everyone';
        
        if (messageSetting === 'everyone') {
            canMessage = true;
        } else if (messageSetting === 'followers') {
            // Check if current user follows target
            const { isFollowing } = await import('./follow-system.js');
            const isFollower = await isFollowing(targetUserId);
            
            if (isFollower) {
                canMessage = true;
            } else {
                canMessage = false;
                messageReason = 'Only followers can message';
            }
        } else if (messageSetting === 'nobody') {
            canMessage = false;
